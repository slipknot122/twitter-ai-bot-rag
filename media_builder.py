...
class MediaBuilder:
    ...
    def generate(self, draft_id: int, prompt: str, token: Optional[str] = None) -> Optional[dict]:
        ...
        if token:
            ...
            tmp_filename = f"draft_{draft_id}_{safe_token}.tmp.jpg"
        else:
            short_uuid = uuid.uuid4().hex[:8]
            filename = f"draft_{draft_id}_{short_uuid}.jpg"
            tmp_filename = f"draft_{draft_id}_{short_uuid}.tmp.jpg"
            
        final_path = self.media_dir / filename
        tmp_path = self.media_dir / tmp_filename
        ...
        for provider_name in provider_order:
            ...
            try:
                ...
                # Валідуємо і зберігаємо в tmp_path
                ...
                logger.error(f"MediaBuilder: Content rejection from {provider_name}")
                last_error = "Content Rejection"
                if tmp_path.exists():
                    tmp_path.unlink(missing_ok=True)
                break

            except ProviderAuthError as e:
                logger.warning(f"MediaBuilder: Auth error from {provider_name}")
                ...
                logger.warning(f"MediaBuilder: Transient error from {provider_name}")
                ...
                logger.error(f"MediaBuilder: Unexpected error from {provider_name}")
    ...
    def delete_media_file(self, media_path: str) -> bool:
        ...
        # Захист від path traversal і symlinks
        try:
            ...
            if file_path.is_symlink():
                logger.error(f"MediaBuilder: Symlink attempt blocked: {media_path}")
                return False
        ...
        try:
            ...
            logger.error(f"MediaBuilder: Failed to delete media file")
            return False
